...
library ForgeArtifacts {
    ...
    function _stripSemver(
        string memory _name
    ) internal returns (string memory out_) {
    ...
    function _getFullyQualifiedName(
        string memory _name
    ) internal returns (string memory out_) {
    ...
    function getStorageLayout(
        string memory _name
    ) public returns (string memory layout_) {
        string[] memory cmd = new string[](3);
        cmd[0] = Executables.bash;
        cmd[1] = "-c";
        cmd[2] = string.concat(
    ...
    function getMethodIdentifiers(
        string memory _name
    ) internal returns (string[] memory ids_) {
        string[] memory cmd = new string[](3);
        cmd[0] = Executables.bash;
        cmd[1] = "-c";
        cmd[2] = string.concat(
            Executables.jq,
            " '.methodIdentifiers | keys' < ",
            _getForgeArtifactPath(_name)
        );
        bytes memory res = vm.ffi(cmd);
    ...
    function _getForgeArtifactDirectory(
        string memory _name
    ) internal returns (string memory dir_) {
        string[] memory cmd = new string[](3);
        cmd[0] = Executables.bash;
        cmd[1] = "-c";
        cmd[2] = string.concat(
    ...
    function _getForgeArtifactPath(
        string memory _name
    ) internal returns (string memory out_) {
    ...
    function getInitializedSlot(
        string memory _contractName
    ) internal returns (StorageSlot memory slot_) {
    ...
    function ensurePath(string memory _path) internal {
        (, bytes memory returndata) = address(vm).call(
            abi.encodeWithSignature("split(string,string)", _path, string("/"))
        );
