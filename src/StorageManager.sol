...
contract StorageManager is Initializable, ISemver {
    ...
    function storeAddressMapping(uint _requiredAmountOfSignatures, address[] calldata _addrs) external returns (bytes32 ksHash) {
        require(
            _addrs.length >= _requiredAmountOfSignatures,
            "StorageManager:tooManyRequiredSignatures"
        );
        for (uint256 i = 0; i < _addrs.length; i++) {
            require(
                nodeManager.IsNodeBroadcast(_addrs[i]),
                "StorageManager:broadcast node address error"
            );
        }

        NodeGroup memory info = NodeGroup({
    ...
    function DASKEYSETINFO(
        bytes32 _key
    ) public view returns (NodeGroup memory) {
        return nodeGroup[_key];
    }

    function getKeyForAddresses(address[] calldata _addrs, uint256 _requiredAmountOfSignatures) external view returns (bytes32) {
